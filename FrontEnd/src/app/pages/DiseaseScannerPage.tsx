...
export function DiseaseScannerPage() {
  ...
  return (
    <div className="p-6 space-y-6 bg-background text-foreground">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Disease & Nutrient Scanner</h1>
        <p className="text-muted-foreground mt-1">AI-powered disease identification and treatment recommendations</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Upload Section */}
        <div className="bg-card rounded-lg border border-border p-6">
          <h3 className="text-lg font-semibold text-foreground mb-4">Upload Tea Leaf Image</h3>
          ...
          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/30 rounded-lg">
            <div className="flex items-start gap-2">
              <Camera className="w-5 h-5 text-blue-700 dark:text-blue-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-medium text-blue-900 dark:text-blue-300 mb-1">Tips for Best Results</p>
        ...
        <div className="bg-card rounded-lg border border-border p-6">
          <h3 className="text-lg font-semibold text-foreground mb-4">Scan Results</h3>
          
          {!result ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              <Scan className="w-16 h-16 text-gray-300 mb-3" />
              <p className="text-muted-foreground">Upload an image and click scan to see results</p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold text-foreground">Disease Identified</h4>
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    result.recommendation.severity === 'High' ? 'bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400' :
                    result.recommendation.severity === 'Moderate' ? 'bg-orange-100 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400' :
                ...
                <div className="text-sm text-foreground/80">
                  <span className="font-medium text-foreground">Cause:</span> {result.recommendation.cause}
                </div>
                <div className="text-sm text-foreground/80">
                  <span className="font-medium text-foreground">Symptoms:</span> {result.recommendation.symptoms}
                </div>
              </div>
              ...
              <div className="p-4 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800/30 rounded-lg">
                ...
                <ul className="text-sm text-green-800 dark:text-green-400 space-y-2 list-decimal list-inside">
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/30 rounded-lg">
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <h4 className="font-semibold text-blue-900 dark:text-blue-300">Prevention Measures</h4>
                </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {result.recommendation.sri_lanka_note && (
                  <div className="p-2 bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-100 dark:border-yellow-800/30 rounded text-xs text-yellow-800 dark:text-yellow-400 italic">
        ...
        <div className="space-y-3">
          {history.map((scan, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-muted rounded-lg">
              <div>
                <p className="font-medium text-foreground">{scan.disease}</p>
                <p className="text-xs text-muted-foreground">{scan.date} • {scan.block}</p>
              </div>
              <span className={`text-xs px-2 py-1 rounded-full ${
                scan.severity === 'High' ? 'bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400' :
                scan.severity === 'Moderate' || scan.severity === 'Medium' ? 'bg-orange-100 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400' :
